...
def event_to_json_string(event) -> str:
    ...
    if isinstance(event, FunctionToolCallEvent):
        ...
        # Do not mutate event properties; just normalize in serialization
        pass
...
def event_to_dict(event):
    ...
    if isinstance(event, PartStartEvent):
        ...
        effective_tool_call_id = tool_call_mapping.get(event.tool_call_id, event.tool_call_id)
        ...
        return {
            "tool_result": {
                ...
                "content": content,
                # Prefer mapped id if available
                "tool_call_id": effective_tool_call_id or getattr(event.result, 'tool_call_id', None),
...
def dict_to_model_message(data: dict) -> Union[ModelRequest, ModelResponse]:
    ...
    for part_data in data["parts"]:
        if part_data["type"] == "ToolCallPart":
            ...
            parts.append(
                ReasoningPart(reasoning=part_data["content"], part_kind=part_data.get("part_kind", "reasoning"))
            )
